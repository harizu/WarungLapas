    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.penjualans.update", [$penjualan->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('trx_no') ? 'is-invalid' : '' }}" type="text" name="trx_no" id="trx_no" value="{{ old('trx_no', $penjualan->trx_no) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $product)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $penjualan->product->id ?? '') == $id ? 'selected' : '' }}>{{ $product }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qty') ? 'is-invalid' : '' }}" type="number" name="qty" id="qty" value="{{ old('qty', $penjualan->qty) }}" step="1" required>
                @if($errors->has('qty'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_price') ? 'is-invalid' : '' }}" type="text" name="total_price" id="total_price" value="{{ old('total_price', $penjualan->total_price) }}" required>
