...
<div class="card">
    <div class="card-header">
        {{ trans('global.view') }} {{ trans('cruds.pembelian.title') }}
    </div>
            ...
            <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-md-12 justify-content-between">
                            <a href="{{ route('admin.riwayatPesanan.index') }}" class="btn btn-danger"><< Kembali</a>

                            <div class="float-right d-inline">
                                @if(!$order->is_expired && $order->status === $order::STATUS_NEW_ORDER)
                                    <form action="{{ route('admin.riwayatPesanan.cancel', $order->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="PUT">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-danger" value="{{ trans('global.cancelOrder') }}">
                                    </form>
                                @endif
                            </div>
                    </div>
